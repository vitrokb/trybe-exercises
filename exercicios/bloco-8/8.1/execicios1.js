...
const getPlanet = () => {
  ...
  setTimeout(function() {console.log("Returned planet: ", mars)}, 4000);
};
...
const sendMarsTemperature = () => {
  ...
  setTimeout(function() { console.log(`Mars temperature is: ${temp} degree Celsius`) }, messageDelay());
};
...
const sendMarsTemperature = (tempFahrenheit) => {
  const recievedTemp = getMarsTemperature();
  setTimeout(() => { tempFahrenheit(recievedTemp) }, messageDelay());
};
...
const sendMarsTemperature = (message, error) => {
  ...
  const temp = getMarsTemperature();
  if (hit >= 60) {
    setTimeout(() => { message(temp) },messageDelay());
  } else {
    setTimeout(() => { error('Robot is busy') }, messageDelay());
  }
};
